<?php
    ...
    case "DELETE":
        ...
        if ($role == 'anonyme'){ //Empêche la suppression pour les utilisateurs "anonyme"
            deliver_response(401, "Permission non accordée. Veuillez vous connecter", NULL);
            break;
        }
        if($role == 'publisher') { // Limite la suppression d'articles, le publisher peut supprimer que ses articles
            ...
            if(testsErreursSansSucces($auteur)){break;}
            //Pas le même auteur que sur le token
            if (!($auteur == getPropertyFromToken($bearer_token, 'username'))){
                deliver_response(401, "Permission non accordée. Vous pouvez seulement supprimer vos articles", NULL);
                break;
            }
    ...
    case "PATCH":
        ...
        if($role == 'publisher') {
            ...
            if(!empty($_GET['id']) && !empty($data->aimer)) {
                ...
                if(is_null($data->aimer) && !is_bool($data->aimer)) {
            ...
            if(!empty($_GET['id']) && !empty($data->contenu)) {
                $code = getAuteurArticle($linkpdo, $_GET['id']);
                if(!testsErreursSansSucces($code)) { break; }
                //Vérifie s'il modifie un post d'une autre personne.
                ...
                $code = modifierArticle($linkpdo, $_GET['id'], $data->contenu);
                testsErreurs($code, "L'article a bien été modifié, ID : ".$_GET['id'], $code);
            }
        //Un modérateur ou un anonyme ne peut pas voter & ne peut pas modifier un article.
